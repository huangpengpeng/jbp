...
public class LLianPayClient {
    ...
    public LLianPayClient(String priKeyValue, String pubKeyValue) {
    ...
    public String sendRequest(String url, String body) {
        ...
        String sign = LLianPayAccpSignature.getInstance().sign(priKeyValue, body);
        return send(url, body, sign);
    }

    public String send(String url, String body, String sign) {
        if (url == null || "".equals(url)) {
            throw new RuntimeException("请求URL非法");
        }
        ...
        try {
            ...
            if (ret == HttpStatus.SC_OK) {
                ...
                if (!"".equalsIgnoreCase(resSignatureData)) {
                    ...
                    boolean checksign = LLianPayAccpSignature.getInstance().checkSign(pubKeyValue, responseString.toString(), resSignatureData);
                    if (!checksign) {
