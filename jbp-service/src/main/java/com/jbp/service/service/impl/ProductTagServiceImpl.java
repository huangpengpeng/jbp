...
import com.jbp.common.model.product.ProductTag;
import com.jbp.common.model.seckill.SeckillProduct;
import com.jbp.common.request.PageParamRequest;
import com.jbp.common.request.producttag.ProductTagSearchRequest;
import com.jbp.common.response.ProductTagTaskItem;
import com.jbp.common.response.ProductTagsFrontResponse;
...
import com.jbp.service.dao.ProductTagDao;
import com.jbp.service.dao.SeckillProductDao;
import com.jbp.service.service.ProductTagService;
    ...
    */
    ...
    public Boolean editProductTag(ProductTag productTag) {
        ...
        if(CollUtil.isNotEmpty(exitProductTags)){
            ...
            if(CollUtil.isNotEmpty(seamTagNameList)){
                ...
                if(CollUtil.isNotEmpty(otherNameSeam)) throw new CrmebException("标签名称已经存在！");
            }
        }
    ...
    public Boolean deleteProductTag(Integer id) {
        ...
        if(productTagForDelete.getOwner().equals(0)){
            throw new CrmebException("平台内置商品标签，不能删除");
        }
        if(productTagForDelete.getStatus() == 1) throw new CrmebException("先关闭状态后再删除商品标签！");
...
//                        ProductTagsFrontResponse tagsFront = new ProductTagsFrontResponse();
            ...
            LOGGER.info("商品标签，自定义标签:配置数据:{}", JSON.toJSONString(selfProductList));
        if(ObjectUtil.isNotNull(selfProductList) && !selfProductList.isEmpty()){
                for (ProductTag productTag : selfProductList) {
                    ...
                    LOGGER.info("商品标签已经存在的有:leftItems:{}", JSON.toJSONString(leftItems));
                    LOGGER.info("商品标签已经存在的有:underItems:{}", JSON.toJSONString(underItems));
                    ...
                    switch (productTag.getPlayType()){
                        case "product":
                            ...
                            LOGGER.info("商品标签已经存在的有:leftItems-product:{}", JSON.toJSONString(leftItems));
                            LOGGER.info("商品标签已经存在的有:underItems-product:{}", JSON.toJSONString(underItems));
                            continue;
                        case "brand":
                            setProductTagForSelfTagConfig(productTag, leftItems, underItems, selfTag, brandId);
                            LOGGER.info("商品标签已经存在的有:leftItems-brand:{}", JSON.toJSONString(leftItems));
                            LOGGER.info("商品标签已经存在的有:underItems-brand:{}", JSON.toJSONString(underItems));
                            continue;
                        case "merchant":
                            setProductTagForSelfTagConfig(productTag, leftItems, underItems, selfTag, merId);
                            LOGGER.info("商品标签已经存在的有:leftItems-merchant:{}", JSON.toJSONString(leftItems));
                            LOGGER.info("商品标签已经存在的有:underItems-merchant:{}", JSON.toJSONString(underItems));
                            continue;
                        case "category":
                            setProductTagForSelfTagConfig(productTag, leftItems, underItems, selfTag, cateId);
                            LOGGER.info("商品标签已经存在的有:leftItems-category:{}", JSON.toJSONString(leftItems));
                            LOGGER.info("商品标签已经存在的有:underItems-category:{}", JSON.toJSONString(underItems));
                    }

                }
            }
//        }
        LOGGER.info("商品标签数据:{}", JSON.toJSONString(tagsFront));
        return tagsFront;
    }
...
//            String productCacheJSON = redisUtil.get(cacheKey);
//            // Map 中 set key=0的是自定义标签的规则
//            cacheProductTags = JSONObject.parseObject(productCacheJSON, Map.class);
//        }else{
...
//                    List<ProductTagTaskItem> boomGoods = calcProductTagForLastThirtyDaysSalesByOptionUnit(Integer.valueOf(productTag.getPlayProducts()));
                    setProductTagsItemAndSored(productTag, boomGoods, cacheProduct);
                }
            ...
            LOGGER.info("自定义商品标签：查询到的开启的商品标签有：{}", JSON.toJSONString(productTagsForSelf));
            if(!productTagsForSelf.isEmpty()){
                List<ProductTag> selfEffProductList = productTagsForSelf.stream().filter(item -> item.getStartTime().compareTo(CrmebDateUtil.nowDateTime()) <= 0
                        && item.getEndTime().compareTo(CrmebDateUtil.nowDateTime()) > 0).collect(Collectors.toList());
                LOGGER.info("自定义商品标签生效的配置：{}", JSON.toJSONString(selfEffProductList));
...
//                cacheProductTags.put(entry.getKey(), entry.getValue());
            }
//            redisUtil.set(cacheKey, JSONObject.toJSONString(cacheProductTags), crmebConfig.getProductTagCacheMinutes(), TimeUnit.MINUTES);
//        }
...
//        Map<Integer, ProductTagTaskResponse> task1 = calcProductTagForLastThirtyDaysSalesByOptionUnit(10);
//        LOGGER.info("爆品{}:", JSON.toJSONString(task1));
//        Map<Integer, ProductTagTaskResponse> task2 = calcProductTagForMerchantIsSelf();
//        LOGGER.info("自营{}:", JSON.toJSONString(task2));
//        Map<Integer, ProductTagTaskResponse> task3 = calcProductTagForLastThirtyDaysReplyCountByOptionUnit(1);
//        LOGGER.info("热卖{}:", JSON.toJSONString(task3));
//        Map<Integer, ProductTagTaskResponse> task4 = calcProductTagForLastThirtyDaysReplyFiveStartByOptionUnit(1);
//        LOGGER.info("优选{}:", JSON.toJSONString(task4));
//        Map<Integer, ProductTagTaskResponse> task5 = calcProductTagForFreeDelivery();
//        LOGGER.info("包邮{}:", JSON.toJSONString(task5));
//        // 根据次序合并标签属性
//        // 第一个爆品直接存储
//        Map<Integer, ProductTagTaskResponse> allProductTags = new HashMap<>(task1);
//
//        for (Map.Entry<Integer, ProductTagTaskResponse> entry2 : task2.entrySet()) {
//            Integer id2 = entry2.getKey();
//            ProductTagTaskResponse item2 = entry2.getValue();
//            if(allProductTags.containsKey(id2)){
//                allProductTags.get(id2).setIsSelf(task2.get(id2).getIsSelf());
//            }else{
//                allProductTags.put(id2, item2);
//            }
//        }
//        for (Map.Entry<Integer, ProductTagTaskResponse> entry3 : task3.entrySet()) {
//            Integer id3 = entry3.getKey();
//            ProductTagTaskResponse item3 = entry3.getValue();
//            // 合并热卖
//            if(allProductTags.containsKey(id3)){
//                allProductTags.get(id3).setReplayCount(task3.get(id3).getReplayCount());
//            }else{
//                allProductTags.put(id3, item3);
//            }
//        }
//        for (Map.Entry<Integer, ProductTagTaskResponse> entry4 :task4.entrySet()) {
//            // 合并优选
//            Integer id4 = entry4.getKey();
//            ProductTagTaskResponse item4 = entry4.getValue();
//            if(allProductTags.containsKey(id4)){
//                allProductTags.get(id4).setFiveStarCount(task4.get(id4).getFiveStarCount());
//            }else{
//                allProductTags.put(id4, item4);
//            }
//        }
//        for (Map.Entry<Integer, ProductTagTaskResponse> entry5 :task5.entrySet()) {
//            // 合并包邮
//            Integer id5 = entry5.getKey();
//            ProductTagTaskResponse item5 = entry5.getValue();
//            if(allProductTags.containsKey(id5)){
//                allProductTags.get(id5).setFreeDelivery(task5.get(id5).getFreeDelivery());
//            }else{
//                allProductTags.put(id5, item5);
//            }
//        }
//        LOGGER.info("allProductTags:{}", JSON.toJSONString(allProductTags));
//        getProductTags();
        calcProductTagForLastThirtyDaysSalesByOptionUnitByJava(2);
    }
...
//********************************************************** 任务计算逻辑 START **********************************/
    ...
    private void checkProductTagStatusOnLimit(ProductTag productTag, Integer status, Boolean isEdit) {
        if(1 == status){
            ...
            if(isEdit){
                ...
                if(currentTags.isEmpty() && effProductTags.size() >= 5) throw new CrmebException("商品标签同时开启的不得超过5条");
            }else{
                if(effProductTags.size() >= 5) throw new CrmebException("商品标签同时开启的不得超过5条");
            }
        }
    }
    ...
    private List<ProductTagTaskItem> calcProductTagForLastThirtyDaysSalesByOptionUnitByJava(Integer unit){
        ...
        **/
        ...
        LOGGER.info("满足条件的订单:{}", JSON.toJSONString(orders));
        ...
        List<OrderDetail> OrderDetailIsSkillProduct = orderDetails.stream().filter(orderDetail -> orderDetail.getProductType().equals(ProductConstants.PRODUCT_TYPE_SECKILL)).collect(Collectors.toList());
        LOGGER.info("参与秒杀的订单:{}", JSON.toJSONString(OrderDetailIsSkillProduct));
        ...
        if(!OrderDetailIsSkillProduct.isEmpty()){
            ...
            if(!secKillIds.isEmpty()){
                ...
                for (OrderDetail orderDetail : orderDetails) {
                    ...
                    LOGGER.info("参与秒杀的商品:{}", JSON.toJSONString(seckillPs));
                    for (SeckillProduct seckillP : seckillPs) {
                        if(orderDetail.getProductType().equals(ProductConstants.PRODUCT_TYPE_SECKILL) && seckillP.getId().equals(orderDetail.getProductId())){
...
//                    return orderDetail;
//                }).collect(Collectors.toList());
                LOGGER.info("秒杀订单详情数据:{}", JSON.toJSONString(orderDetails));
            }
        }

        // 筛过秒杀订单之后 将秒杀订单销量赚到普通订单的商品之后的数据
//        List<OrderDetail> OrderDetailIsNormalHasSkillOrderPayNum = orderDetails.stream().filter(orderDetail -> orderDetail.getProductType().equals(ProductConstants.PRODUCT_TYPE_NORMAL)).collect(Collectors.toList());
        ...
        LOGGER.info("爆品计算：筛选之前的标签:{}", JSON.toJSONString(itemList));
        itemList = itemList.stream().filter(tag -> tag.getFlag() > unit).collect(Collectors.toList());
        LOGGER.info("爆品计算：最终符合条件的标签：{}", JSON.toJSONString(itemList));
        return itemList;
    }
     ...
     * 再对比当前的配置数量，将符合条件的数据存储到redis后在商品查询时给予标识
     * @param unit 件
     */
    private List<ProductTagTaskItem> calcProductTagForLastThirtyDaysSalesByOptionUnit(Integer unit){
        Map<String, Object> prams = new HashMap<>();
        prams.put("unit", unit);
        return dao.calcProductTagForLastThirtyDaysSalesByOptionUnit(prams);
    }
