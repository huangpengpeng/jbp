 ...
 */
@Component
public class CapaEqualCommHandler extends AbstractProductCommHandler {
    ...
    public Integer getType() {
        return ProductCommEnum.平级佣金.getType();
    }
    ...
    public Boolean saveOrUpdate(ProductComm productComm) {
        ...
        if (productComm.hasError()) {
            throw new CrmebException(ProductCommEnum.平级佣金.getName() + "参数不完整");
        }
    ...
    public void orderSuccessCalculateAmt(Order order, LinkedList<CommCalculateResult> resultList) {
        ...
        // 找到所有平级用户
        List<UserCapa> userEqualList = Lists.newArrayList();
        for (UserCapa userCapa : userList) {
            Boolean start = false;

            for (UserUpperDto upperDto2 : allUpper) {
                if (upperDto2.getPId() == null) { // 没有下级找完了
                    break;
                }
                // 找到自己所在的位置
                if (upperDto2.getPId().intValue() == userCapa.getUid().intValue()) {
                    start = true;
                    continue;
                }
                // 从自己的位置往下找
                if (start) {
                    UserCapa pUserCapa = uidCapaMap.get(upperDto2.getPId());
                    if (pUserCapa.getCapaId().intValue() > userCapa.getCapaId().intValue()) {
                        break;
                    }
                    if (pUserCapa.getCapaId().intValue() == userCapa.getCapaId().intValue()) {
                        userEqualList.add(pUserCapa);
                        break;
                    }
                }
            }
        ...
        // 没人退出
        if (CollectionUtils.isEmpty(userEqualList)) {
            return;
        }
        ...
        for (OrderDetail orderDetail : orderDetails) {
            ...
            // 每个人拿钱
            for (UserCapa userCapa : userEqualList) {
                ...
                // 佣金
                if (ArithmeticUtils.gt(ratio, BigDecimal.ZERO)) {
        ...
        userAmtMap.forEach((uid, amt) -> {
            ...
            if (ArithmeticUtils.gt(clearingFee, BigDecimal.ZERO)) {
                ...
                fundClearingService.create(uid, order.getOrderNo(), ProductCommEnum.平级佣金.getName(), clearingFee,
                        null, fundClearingProducts, orderUser.getAccount() + "下单获得" + ProductCommEnum.平级佣金.getName(), "");
            }
        });
    }
