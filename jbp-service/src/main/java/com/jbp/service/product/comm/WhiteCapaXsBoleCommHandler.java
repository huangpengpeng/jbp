 ...
 */
@Component
public class WhiteCapaXsBoleCommHandler extends AbstractProductCommHandler {
    ...
    public Integer getType() {
        return ProductCommEnum.白名单级差伯乐佣金.getType();
    }
    ...
    public void orderSuccessCalculateAmt(Order order, List<OrderDetail> orderDetails, LinkedList<CommCalculateResult> resultList) {
        ...
        // 星级级差佣金
        List<CommCalculateResult> collisionFeeList = resultList.stream().filter(r -> r.getType().equals(ProductCommEnum.白名单星级级差佣金.getType()))
                    ...
                    if (pCapaXs != null && pCapaXs.getCapaId().intValue() >= rule.getCapaXsId().intValue()) {
                        ...
                        if (ArithmeticUtils.gt(amt, BigDecimal.ZERO)) {
                            fundClearingService.create(upperDto.getPId(), order.getOrderNo(), ProductCommEnum.级差伯乐佣金.getName(), amt,
                                     null, user.getNickname()+"|"+user.getAccount() + "获得星级级差佣金奖励上级" + ProductCommEnum.级差伯乐佣金.getName(), "");
                        }
                        i++;
                    }
                }
