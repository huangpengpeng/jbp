...
import com.jbp.common.request.agent.SelfScoreUpdateRequest;
...
public class SelfScoreController {
    ...
    @PreAuthorize("hasAuthority('agent:self:score:team:update:score')")
    @LogControllerAnnotation(intoDB = true, methodType = MethodType.ADD, description = "个人业绩更新")
    @ApiOperation("更新业绩")
    @PostMapping("/update/score")
    public CommonResult updateScore(@Validated @RequestBody SelfScoreUpdateRequest request) {
        User user = userService.getByAccount(request.getAccount());
        if (user == null) {
            throw new CrmebException("账号信息错误");
        }
        selfScoreService.updateScore(user.getId(), request.getScore(),request.getOrdersSn(),request.getPayTime(),request.getRemark(),request.getIfAdd());
