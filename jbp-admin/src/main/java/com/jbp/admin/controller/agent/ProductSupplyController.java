...
public class ProductSupplyController {
    ...
    @PreAuthorize("hasAuthority('agent:product:supply:supplyList')")
    @GetMapping("/supplyList")
    @ApiOperation("供应商列表")
    public CommonResult<List<ProductSupply>> getSupplyList() {
        return CommonResult.success(productSupplyService.supplyList());
    }

    @PreAuthorize("hasAuthority('agent:product:supply:add')")
    @PostMapping("/add")
    @ApiOperation("新增")
